...
; (function () {
  ...
  function hightLightFontOnPage (e) {
    chrome.tabs.query({ active: true, currentWindow: true }, function (tabs) {
      chrome.tabs.sendMessage(tabs[0].id, { styleId: `${e.target.innerText}-div#header` }, function (response) {
  ...
  async function copyToClipboard (e) {
    ...
    try {
      ...
      createNotification('Copied to Clipboard!', `${text} has been copied to the clipboard.`)
    } catch (err) {
